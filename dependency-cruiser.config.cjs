...
module.exports = {
  forbidden: [
    ...
    {
      name: "v1-authoring-depends-only-on-v1-contracts",
      severity: "error",
      from: { path: "(^|/)packages/authoring/" },
      to: { path: "(^|/)packages/(?!(view|transformation|execution|view-package|authoring)/)" },
    },
    {
    ...
    {
      name: "v1-search-depends-only-on-view",
      severity: "error",
      from: { path: "(^|/)packages/search/" },
      to: { path: "(^|/)packages/(?!(view|search)/)" },
    },
    {
      name: "v1-operations-depends-only-on-v1-ports",
      severity: "error",
      from: { path: "(^|/)packages/operations/" },
      to: { path: "(^|/)packages/(?!(view|transformation|execution|capture|search|authoring|operations)/)" },
    },
    {
      name: "v1-screenpipe-contracts-depends-only-on-view",
      severity: "error",
      from: { path: "(^|/)packages/screenpipe-contracts/" },
      to: { path: "(^|/)packages/(?!(view|screenpipe-contracts)/)" },
    },
    {
      name: "v1-adapters-depend-only-on-v1-ports",
      severity: "error",
      from: { path: "(^|/)packages/adapters/" },
      to: { path: "(^|/)packages/(?!(view|transformation|execution|automation|capture|search|authoring|operations|screenpipe-contracts|adapters)/)" },
    },
    ...adapterIndependenceRules,
    {
      name: "v1-packages-do-not-import-composition-roots",
      severity: "error",
      from: { path: "(^|/)(?:packages/(view|view-package|transformation|execution|automation|capture|search|authoring|operations|screenpipe-contracts|adapters)|view-packages)/" },
