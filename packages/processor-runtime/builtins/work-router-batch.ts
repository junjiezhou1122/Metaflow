...
export function createWorkRouterBatchProcessor(options: { now?: Date; minutes?: number; limit?: number } = {}): ProcessorDefinition {
  return {
...
export function workRouterBatchHandler(options: { now?: Date; minutes?: number; limit?: number } = {}): ProcessorHandler {
  return (_input, context) => {
    const result = compileWorkFocusSet({ ...options, write: false }, context.store);
    const { view_type: _vt, ...rest } = result.view as ContextView;
    return {
      views: [{
        ...rest,
        type: WORK_FOCUS_SET_VIEW_TYPE,
        metadata: { ...rest.metadata, records_scanned: result.records_scanned, route_candidates_used: result.route_candidates_used },
      }],
