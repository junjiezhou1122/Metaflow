import type {
  ...
  AutomationSurfaceRenderer,
} from "@info/automation";

export type BrowserDeliveryCard = {
  delivery_id: string;
  request: AutomationDeliveryRequest;
  rendered_at: string;
};

export class BrowserDeliveryMailbox implements AutomationSurfaceRenderer {
  readonly surface = "browser";
  readonly capacity = "multiple" as const;
  private readonly cards = new Map<string, BrowserDeliveryCard>();

  constructor(private readonly now: () => Date = () => new Date()) {}

  async render(request: AutomationDeliveryRequest): Promise<{ delivery_id: string }> {
    if (request.surface !== this.surface) {
      throw new Error(`Browser renderer cannot render surface: ${request.surface}`);
    }
    const deliveryId = `browser-delivery:${request.id}`;
    const existing = this.cards.get(deliveryId);
    if (existing) {
      if (JSON.stringify(existing.request) !== JSON.stringify(request)) {
        throw new Error(`Browser delivery id conflict: ${deliveryId}`);
      }
      return { delivery_id: deliveryId };
    }
    this.cards.set(deliveryId, {
  ...
  async withdraw(input: {
    ...
    if (!card) throw new Error(`Browser delivery is missing: ${input.delivery_id}`);
    if (card.request.id !== input.request_id) {
      throw new Error(`Browser delivery request mismatch: ${input.delivery_id}`);
    }
    this.cards.delete(input.delivery_id);
  }

  list(input: { after?: string; limit?: number } = {}): BrowserDeliveryCard[] {
