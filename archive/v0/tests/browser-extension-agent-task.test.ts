...
test("Chrome ACP Tasks view consumes v1 Ambient deliveries and exact result Views", () => {
  const tasks = readFileSync("apps/chrome-acp/packages/chrome-extension/src/components/TasksView.tsx", "utf8");
  const capture = readFileSync("apps/chrome-acp/packages/chrome-extension/src/lib/info-capture.ts", "utf8");

  assert.match(tasks, /type:\s*"poll-ambient-deliveries"/);
  assert.match(tasks, /type:\s*"get-ambient-exact-view"/);
  assert.match(tasks, /type:\s*"ambient-delivery-interaction"/);
  assert.match(tasks, /card\.request\.actions\.includes\("accept"\)/);
  assert.match(tasks, /card\.request\.actions\.includes\("later"\)/);
  assert.match(tasks, /card\.request\.actions\.includes\("dismiss"\)/);
  assert.match(capture, /if \(message\?\.type === "poll-ambient-deliveries"\)/);
  assert.match(capture, /if \(message\?\.type === "get-ambient-exact-view"\)/);
  assert.match(capture, /if \(message\?\.type === "ambient-delivery-interaction"\)/);
});
